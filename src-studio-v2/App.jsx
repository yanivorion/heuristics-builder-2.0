...
import { ReportHeuristicBugWizard } from './components/feedback/ReportHeuristicBugWizard'
import { RequestHeuristicWizard } from './components/feedback/RequestHeuristicWizard'
...
const ACTION_REMAP = {
  ...
  'Pin Item': 'Set Pinned',
  'Set Margins': 'Set Margin',
  'Set Paddings': 'Set Padding',
  'Set Rotations': 'Set Rotation',
  'Set Sizes': 'Set Size',
  'Set Alignments': 'Set Alignment'
}

// Columns whose values can hold multiple canonicals separated by " + " (legacy)
// or " / " (legacy WHEN labels). Normalising them to " | " at load time keeps
// the chip rendering, filtering, and dropdown extraction in lockstep.
const MULTI_VALUE_DB_COLUMNS = ['when', 'in', 'action', 'category']

function normalizeMultiValueSeparators(value) {
...
function transformFromDb(row) {
  ...
  for (const col of MULTI_VALUE_DB_COLUMNS) {
    if (out[col]) out[col] = normalizeMultiValueSeparators(out[col])
  }
...
function App() {
  ...
  const [showBugWizard, setShowBugWizard] = useState(false)
  ...
  // Cells like `when: "Hamburger Menu | Shape"` or `in: "Header | System Container"`
  // pack multiple canonicals into one string. The filters compare per-segment
  // so picking "Shape" or "Header" still surfaces those rows.
  const matchesMulti = useCallback((cellValue, filterValue) => {
    if (!filterValue) return true
    if (cellValue == null) return false
    return String(cellValue)
      .split(/\s*\|\s*/)
      .map(s => s.trim())
      .some(s => s === filterValue)
  }, [])
  ...
  const filteredRows = useMemo(() => {
    ...
    if (filterCategory) result = result.filter(r => matchesMulti(r.category, filterCategory))
    if (filterAction) result = result.filter(r => matchesMulti(r.action, filterAction))
    if (filterContext) result = result.filter(r => matchesMulti(r.in, filterContext))
    ...
    return result
  }, [rows, search, filterCategory, filterAction, filterContext, filterPriority, filterStatus, sortKey, sortDir, matchesMulti])
  ...
  const filteredHeaderRows = useMemo(() => {
    ...
    if (headerFilterCategory) result = result.filter(r => matchesMulti(r.category, headerFilterCategory))
    if (headerFilterAction) result = result.filter(r => matchesMulti(r.action, headerFilterAction))
    ...
    return result
  }, [headerRows, headerSearch, headerFilterCategory, headerFilterAction, headerFilterStatus, headerSortKey, headerSortDir, matchesMulti])

  const mappedCount = rows.filter(r => r.status === 'Mapped').length
  const needsCount = rows.filter(r => r.status === 'Needs Values').length

  // Flatten pipe-separated values into individual options so the dropdowns
  // expose every canonical separately, matching how `matchesMulti` filters.
  const flattenMulti = useCallback(values => {
    const out = new Set()
    for (const v of values) {
      if (!v) continue
      for (const p of String(v).split(/\s*\|\s*/).map(s => s.trim()).filter(Boolean)) out.add(p)
    }
    return [...out].sort((a, b) => String(a).localeCompare(String(b)))
  }, [])

  // Pull dropdown options straight from the loaded rows so any DB value that
  // slipped past the canonical lists still appears as a filter choice. We
  // union with the canonical list to keep empty datasets useful.
  const categoriesFlat = useMemo(
    () => flattenMulti([...rows.map(r => r.category), ...CATEGORIES]),
    [rows, flattenMulti]
  )
  const actionsFlat = useMemo(
    () => flattenMulti([...rows.map(r => r.action), ...ACTIONS]),
    [rows, flattenMulti]
  )
  const contextsFlat = useMemo(
    () => flattenMulti([...rows.map(r => r.in), ...CONTEXTS]),
    [rows, flattenMulti]
  )
  const headerCategories = useMemo(() => flattenMulti(headerRows.map(r => r.category)), [headerRows, flattenMulti])
  const headerActions = useMemo(() => flattenMulti(headerRows.map(r => r.action)), [headerRows, flattenMulti])
  ...
  return (
    <>
          ...
          <div className="toolbar">
            <div className="toolbar-left">
              ...
              >
                ...
                {(activeTab === 'header' ? headerCategories : categoriesFlat).map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <select
                className="filter-select"
              ...
              >
                ...
                {(activeTab === 'header' ? headerActions : actionsFlat).map(a => <option key={a} value={a}>{a}</option>)}
              </select>
              {activeTab === 'main' && (
                <>
                  <select className="filter-select" value={filterContext} onChange={e => setFilterContext(e.target.value)}>
                    <option value="">All Contexts</option>
                    {contextsFlat.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
            ...
            <div className="toolbar-right">
              ...
              <button type="button" className="btn btn-ghost" onClick={() => setShowBugWizard(true)} title="Report a bug on an existing heuristic">Report a bug</button>
              <button type="button" className="btn btn-ghost" onClick={() => setShowRequestWizard(true)} title="Request a new heuristic">Request a heuristic</button>
      ...
      <ReportHeuristicBugWizard
        open={showBugWizard}
        onClose={() => setShowBugWizard(false)}
        onSubmit={async (payload) => {
          // Persistence is intentionally out of scope for the wizard. For
          // now we log the structured payload so a backend integration
          // (Linear, GitHub, Supabase, etc.) can be wired in later.
          console.info('[heuristic bug report]', payload)
          toast('Bug report submitted. Thanks!', 'success')
        }}
      />

      <RequestHeuristicWizard
        open={showRequestWizard}
        onClose={() => setShowRequestWizard(false)}
        onSubmit={async (payload) => {
          console.info('[new heuristic request]', payload)
          toast('Heuristic request submitted. Thanks!', 'success')
        }}
      />
    </>
  )
